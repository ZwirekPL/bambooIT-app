...
import { labPanelRouter } from './routes/labpanel.routes';
...
import { foodProductRouter } from './routes/foodProduct.routes';
import { recipeRouter } from './routes/recipe.routes';
import { checkinRouter } from './routes/checkin.routes';
import { testimonialRouter } from './routes/testimonial.routes';
import { dietitianRouter } from './routes/dietitian.routes';
import { noteRouter } from './routes/note.routes';
import { noteTemplateRouter } from './routes/noteTemplate.routes';
import { referralRouter } from './routes/referral.routes';
import { cleanProductRouter } from './routes/cleanProduct.routes';
...
app.use('/interviews', requireAuth(), userLimiter, interviewRouter);
app.use('/diet-plans', requireAuth(), userLimiter, dietPlanRouter);
app.use('/tenants', tenantRouter);
app.use('/patients', requireAuth('ADMIN', 'DIETITIAN'), userLimiter, patientRouter);
app.use('/profile', requireAuth(), userLimiter, profileRouter);
app.use('/admin', requireAuth('ADMIN'), userLimiter, adminRouter);
app.use('/access', requireAuth(), userLimiter, accessRouter);
app.use('/checkout', requireAuth(), userLimiter, checkoutRouter);
app.use('/orders', requireAuth('ADMIN', 'DIETITIAN', 'PATIENT'), userLimiter, orderRouter);
app.use('/lab-panels', requireAuth('ADMIN', 'DIETITIAN'), userLimiter, labPanelRouter);
app.use('/subscriptions', requireAuth('ADMIN', 'DIETITIAN'), userLimiter, subscriptionRouter);
app.use('/dietitian', requireAuth('ADMIN', 'DIETITIAN'), userLimiter, dietitianRouter);
app.use('/notes', requireAuth('ADMIN', 'DIETITIAN'), userLimiter, noteRouter);
app.use('/note-templates', requireAuth('ADMIN', 'DIETITIAN'), userLimiter, noteTemplateRouter);
app.use('/posts', globalLimiter, blogRouter);
app.use('/food-products', globalLimiter, foodProductRouter);
app.use('/clean-products', globalLimiter, cleanProductRouter);
app.use('/recipes', globalLimiter, recipeRouter);
app.use('/meals', globalLimiter, mealRouter);
app.use('/template-plans', globalLimiter, templateRouter);
app.use('/check-ins', requireAuth(), userLimiter, checkinRouter);
app.use('/progress', requireAuth(), userLimiter, checkinRouter); // 62.1: alias
app.use('/testimonials', globalLimiter, testimonialRouter);
app.use('/referrals', requireAuth(), userLimiter, referralRouter);
app.use('/onboarding', requireAuth(), userLimiter, onboardingRouter);
app.use('/ratings', requireAuth(), userLimiter, ratingRouter);
app.use('/messages', userLimiter, messageRouter);
app.use('/measurements', userLimiter, measurementRouter);
app.use('/supplements', requireAuth(), userLimiter, supplementRouter);
