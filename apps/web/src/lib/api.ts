...
export const api = {
  ...
  dietPlans: {
    ...
    getShoppingList: (planId: string, token: string, days?: number[]) => {
      const query = days ? `?days=${days.join(',')}` : '';
      return apiFetch<{ ok: boolean; categories: import('@/types/api').ShoppingListCategory[] }>(`/diet-plans/${planId}/shopping-list${query}`, { token });
    },
    getShoppingListChecks: (planId: string, token: string) =>
      apiFetch<{ ok: boolean; checks: Record<string, boolean> }>(`/diet-plans/${planId}/shopping-list/checks`, { token }),
    putShoppingListChecks: (planId: string, items: Array<{ name: string; checked: boolean }>, token: string) =>
      apiFetch<{ ok: boolean }>(`/diet-plans/${planId}/shopping-list/checks`, { method: 'PUT', body: JSON.stringify({ items }), token }),
  ...
  adminFoodProducts: {
    list: (params: { page?: number; limit?: number; search?: string; categoryId?: string }, token: string) => {
      const query = new URLSearchParams();
      if (params.page) query.set('page', String(params.page));
      if (params.limit) query.set('limit', String(params.limit));
      if (params.search) query.set('search', params.search);
      if (params.categoryId) query.set('categoryId', params.categoryId);
      return apiFetch<{ ok: boolean; items: FoodProduct[]; total: number; page: number; limit: number }>(
        `/admin/food-products?${query.toString()}`,
        { token }
      );
    },
    getById: (id: string, token: string) =>
      apiFetch<{ ok: boolean; item: FoodProduct }>(`/admin/food-products/${id}`, { token }),
    search: (q: string, token: string) =>
      apiFetch<{ ok: boolean; items: FoodProduct[] }>(`/admin/food-products/search?q=${encodeURIComponent(q)}`, { token }),
    create: (data: FoodProductCreateData, token: string) =>
      apiFetch<{ ok: boolean; item: FoodProduct }>('/admin/food-products', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    update: (id: string, data: Partial<FoodProductCreateData>, token: string) =>
      apiFetch<{ ok: boolean; item: FoodProduct }>(`/admin/food-products/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
        token,
      }),
    remove: (id: string, token: string) =>
      apiFetch<{ ok: boolean }>(`/admin/food-products/${id}`, { method: 'DELETE', token }),
    verify: (id: string, token: string) =>
      apiFetch<{ ok: boolean; item: FoodProduct }>(`/admin/food-products/${id}/verify`, {
        method: 'PATCH',
        token,
      }),
    categories: (token: string) =>
      apiFetch<{ ok: boolean; categories: FoodProductCategory[] }>('/admin/food-products/categories', { token }),
  },
  adminCleanProducts: {
    list: (params: { page?: number; limit?: number; search?: string; type?: CleanProductType; category?: string; verificationStatus?: string; minQuality?: number; maxQuality?: number }, token: string) => {
      const query = new URLSearchParams();
      if (params.page) query.set('page', String(params.page));
      if (params.limit) query.set('limit', String(params.limit));
      if (params.search) query.set('search', params.search);
      ...
      if (params.minQuality !== undefined) query.set('minQuality', String(params.minQuality));
      if (params.maxQuality !== undefined) query.set('maxQuality', String(params.maxQuality));
      return apiFetch<{ ok: boolean; items: CleanProduct[]; total: number; page: number; limit: number }>(
        `/admin/clean-products?${query.toString()}`,
        { token }
      );
    },
    getById: (id: string, token: string) =>
      apiFetch<{ ok: boolean; item: CleanProduct }>(`/admin/clean-products/${id}`, { token }),
    search: (q: string, token: string, type?: CleanProductType, category?: string) => {
      const params = new URLSearchParams({ q });
      if (type) params.set('type', type);
      if (category) params.set('category', category);
      return apiFetch<{ ok: boolean; items: CleanProduct[] }>(
        `/admin/clean-products/search?${params.toString()}`,
        { token }
      );
    },
    categories: (token: string) =>
      apiFetch<{ ok: boolean; categories: string[] }>(
        '/admin/clean-products/categories',
        { token }
      ),
    stats: (token: string) =>
      apiFetch<{ ok: boolean; stats: { total: number; byType: Record<string, number>; byVerification: Record<string, number>; withMicro: number; withoutMicro: number; avgQualityScore: number } }>(
        '/admin/clean-products/stats',
        { token }
      ),
    create: (data: Record<string, unknown>, token: string) =>
      apiFetch<{ ok: boolean; item: CleanProduct }>('/admin/clean-products', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    update: (id: string, data: Record<string, unknown>, token: string) =>
      apiFetch<{ ok: boolean; item: CleanProduct }>(`/admin/clean-products/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
        token,
      }),
    remove: (id: string, token: string) =>
      apiFetch<{ ok: boolean }>(`/admin/clean-products/${id}`, { method: 'DELETE', token }),
    previewNutrition: (ingredients: Array<{ cleanProductId: string; grams: number }>, servings: number, token: string) =>
      apiFetch<{ ok: boolean } & NutritionPreview>('/admin/clean-products/preview-nutrition', {
        method: 'POST',
        body: JSON.stringify({ ingredients, servings }),
        token,
      }),
    bulk: (data: { ids: string[]; action: string; category?: string }, token: string) =>
      apiFetch<{ ok: boolean; affected: number }>('/admin/clean-products/bulk', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    history: (id: string, token: string) =>
      apiFetch<{ ok: boolean; logs: AuditLogEntry[] }>(`/admin/clean-products/${id}/history`, { token }),
    duplicates: (params: { page?: number; limit?: number; category?: string }, token: string) => {
      const query = new URLSearchParams();
      if (params.page) query.set('page', String(params.page));
      if (params.limit) query.set('limit', String(params.limit));
      if (params.category) query.set('category', params.category);
      return apiFetch<{ ok: boolean; groups: DuplicateGroup[]; total: number }>(
        `/admin/clean-products/duplicates?${query.toString()}`,
        { token }
      );
    },
    merge: (data: { targetId: string; sourceId: string; fieldsFromSource?: string[] }, token: string) =>
      apiFetch<{ ok: boolean; item: CleanProduct }>('/admin/clean-products/merge', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
  ...
  adminRecipes: {
    list: (params: Record<string, unknown>, token: string) => {
      const query = new URLSearchParams();
      for (const [k, v] of Object.entries(params)) {
        if (v !== undefined && v !== null && v !== '') query.set(k, String(v));
      }
      return apiFetch<{ ok: boolean; items: Recipe[]; total: number; page: number; limit: number }>(
        `/admin/recipes?${query.toString()}`,
        { token }
      );
    },
    getById: (id: string, token: string) =>
      apiFetch<{ ok: boolean; item: Recipe }>(`/admin/recipes/${id}`, { token }),
    create: (data: RecipeCreateData, token: string) =>
      apiFetch<{ ok: boolean; item: Recipe }>('/admin/recipes', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    update: (id: string, data: Partial<RecipeCreateData>, token: string) =>
      apiFetch<{ ok: boolean; item: Recipe }>(`/admin/recipes/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
        token,
      }),
    remove: (id: string, token: string) =>
      apiFetch<{ ok: boolean }>(`/admin/recipes/${id}`, { method: 'DELETE', token }),
    bulk: (data: { ids: string[]; action: string; value?: unknown }, token: string) =>
      apiFetch<{ ok: boolean; updated: number; skipped: number; errors: string[] }>('/admin/recipes/bulk', {
        method: 'PATCH',
        body: JSON.stringify(data),
        token,
      }),
    findDuplicates: (token: string, minSimilarity = 80) =>
      apiFetch<{ ok: boolean; clusters: Array<{ recipes: Array<{ id: string; title: string; qualityScore: number; category: string | null; mealType: string | null }>; similarity: number }>; totalClusters: number }>(
        `/admin/recipes/find-duplicates?minSimilarity=${minSimilarity}`,
        { method: 'POST', token }
      ),
    merge: (data: { masterId: string; duplicateIds: string[] }, token: string) =>
      apiFetch<{ ok: boolean; merged: number }>('/admin/recipes/merge', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    bulkRecomputeNutrition: (mode: 'missing' | 'broken' | 'missing_micro' | 'all', token: string) =>
      apiFetch<{ ok: boolean; total: number; success: number; failed: number; improved: number }>(
        '/admin/recipes/bulk-recompute-nutrition',
        { method: 'POST', body: JSON.stringify({ mode }), token }
      ),
    dataQualityReport: (token: string) =>
      apiFetch<{
        ok: boolean;
        report: {
          totalRecipes: number;
      ...
      }>('/admin/recipes/data-quality-report', { token }),
  },
  ratings: {
    rate: (data: { recipeId: string; rating: number; comment?: string; dietPlanId?: string; source?: string }, token: string) =>
      apiFetch<{ ok: boolean; rating: { id: string; rating: number } }>('/ratings', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    batchRate: (data: { ratings: Array<{ recipeId: string; rating: number; comment?: string }>; dietPlanId?: string }, token: string) =>
      apiFetch<{ ok: boolean }>('/ratings/batch', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    my: (token: string, dietPlanId?: string) => {
      const query = dietPlanId ? `?dietPlanId=${dietPlanId}` : '';
      return apiFetch<{ ok: boolean; ratings: Array<{ id: string; recipeId: string; rating: number; comment?: string; recipe: { id: string; title: string } }> }>(
        `/ratings/my${query}`,
        { token }
      );
    },
    forRecipe: (recipeId: string, token: string) =>
      apiFetch<{ ok: boolean; ratings: Array<{ id: string; rating: number; comment?: string; patient: { firstName?: string; lastName?: string } }> }>(
        `/ratings/recipe/${recipeId}`,
        { token }
      ),
    top: (token: string, limit = 20) =>
      apiFetch<{ ok: boolean; recipes: Array<{ id: string; title: string; averageRating: number; ratingCount: number }> }>(
        `/ratings/top?limit=${limit}`,
        { token }
      ),
    low: (token: string, limit = 20) =>
      apiFetch<{ ok: boolean; recipes: Array<{ id: string; title: string; averageRating: number; ratingCount: number }> }>(
        `/ratings/low?limit=${limit}`,
        { token }
      ),
    toggleFavorite: (data: { recipeName: string; recipeId?: string; dietPlanId?: string }, token: string) =>
      apiFetch<{ ok: boolean; favorited: boolean }>('/ratings/favorite', { method: 'POST', body: JSON.stringify(data), token }),
    listFavorites: (token: string) =>
      apiFetch<{ ok: boolean; favorites: string[] }>('/ratings/favorites', { token }),
  },
  foodProducts: {
    list: (params: { page?: number; limit?: number; search?: string; categoryId?: string }, token: string) => {
      const query = new URLSearchParams();
      ...
      if (params.categoryId) query.set('categoryId', params.categoryId);
      return apiFetch<{ ok: boolean; items: FoodProduct[]; total: number; page: number; limit: number }>(
        `/dietitian/food-products?${query.toString()}`,
        { token }
      );
    },
    getById: (id: string, token: string) =>
      apiFetch<{ ok: boolean; item: FoodProduct }>(`/dietitian/food-products/${id}`, { token }),
    search: (q: string, token: string) =>
      apiFetch<{ ok: boolean; items: FoodProduct[] }>(`/dietitian/food-products/search?q=${encodeURIComponent(q)}`, { token }),
    create: (data: FoodProductCreateData, token: string) =>
      apiFetch<{ ok: boolean; item: FoodProduct }>('/dietitian/food-products', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    update: (id: string, data: Partial<FoodProductCreateData>, token: string) =>
      apiFetch<{ ok: boolean; item: FoodProduct }>(`/dietitian/food-products/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
        token,
      }),
    categories: (token: string) =>
      apiFetch<{ ok: boolean; categories: FoodProductCategory[] }>('/dietitian/food-products/categories', { token }),
  },
  recipes: {
    list: (params: Record<string, unknown>, token: string) => {
      const query = new URLSearchParams();
      ...
      return apiFetch<{ ok: boolean; items: Recipe[]; total: number; page: number; limit: number }>(
        `/dietitian/recipes?${query.toString()}`,
        { token }
      );
    },
    search: (q: string, token: string) =>
      apiFetch<{ ok: boolean; recipes: Recipe[] }>(`/dietitian/recipes/search?q=${encodeURIComponent(q)}`, { token }),
    getById: (id: string, token: string) =>
      apiFetch<{ ok: boolean; item: Recipe }>(`/dietitian/recipes/${id}`, { token }),
    create: (data: RecipeCreateData, token: string) =>
      apiFetch<{ ok: boolean; item: Recipe }>('/dietitian/recipes', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
    update: (id: string, data: Partial<RecipeCreateData>, token: string) =>
      apiFetch<{ ok: boolean; item: Recipe }>(`/dietitian/recipes/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
        token,
      }),
  },
  invitePatient: (email: string, token: string) =>
    apiFetch<{ ok: boolean }>('/dietitian/invite-patient', {
      method: 'POST',
  ...
  cleanProducts: {
    search: (q: string, token: string, type?: CleanProductType, category?: string) => {
      ...
      if (category) params.set('category', category);
      return apiFetch<{ ok: boolean; items: CleanProduct[] }>(
        `/dietitian/clean-products/search?${params.toString()}`,
        { token }
      );
    },
    categories: (token: string) =>
      apiFetch<{ ok: boolean; categories: string[] }>(
        '/dietitian/clean-products/categories',
        { token }
      ),
    list: (params: { page?: number; limit?: number; search?: string; type?: CleanProductType; category?: string }, token: string) => {
      const query = new URLSearchParams();
      if (params.page) query.set('page', String(params.page));
      if (params.limit) query.set('limit', String(params.limit));
      if (params.search) query.set('search', params.search);
      ...
      if (params.category) query.set('category', params.category);
      return apiFetch<{ ok: boolean; items: CleanProduct[]; total: number; page: number; limit: number }>(
        `/dietitian/clean-products?${query.toString()}`,
        { token }
      );
    },
    getById: (id: string, token: string) =>
      apiFetch<{ ok: boolean; item: CleanProduct }>(`/dietitian/clean-products/${id}`, { token }),
    previewNutrition: (ingredients: Array<{ cleanProductId: string; grams: number }>, servings: number, token: string) =>
      apiFetch<{ ok: boolean } & NutritionPreview>('/dietitian/clean-products/preview-nutrition', {
        method: 'POST',
        body: JSON.stringify({ ingredients, servings }),
        token,
      }),
    dietitianCreate: (data: { name: string; nameEn?: string; category: string; subcategory?: string; description?: string; nutrients: { kcalPer100g: number; proteinPer100g: number; fatPer100g: number; carbsPer100g: number }; portions?: Array<{ portionName: string; weightG: number }> }, token: string) =>
      apiFetch<{ ok: boolean; item: CleanProduct }>('/dietitian/clean-products', {
        method: 'POST',
        body: JSON.stringify(data),
        token,
      }),
